...
import com.onetwodevs.anysend.dialog.EstablishConnectionDialog;
import com.onetwodevs.anysend.app.Activity;
import com.onetwodevs.anysend.database.AccessDatabase;
import com.onetwodevs.anysend.dialog.SelectAssigneeDialog;
import com.onetwodevs.anysend.dialog.ToggleMultipleTransferDialog;
import com.onetwodevs.anysend.dialog.TransferInfoDialog;
import com.onetwodevs.anysend.fragment.TransferFileExplorerFragment;
...
public class ViewTransferActivity
    ...
    public static final String ACTION_LIST_TRANSFERS = "com.genonbeta.anysend.action.LIST_TRANSFERS";
    ...
    public void setupSharedPrefs() {
        sharedPreferences = getSharedPreferences(AppUtils.APP_PREFS, MODE_PRIVATE);
        editor = sharedPreferences.edit();
    }

    public boolean getSharedPrefs() {
        return sharedPreferences.getBoolean(AppUtils.showagain, true);
    }
    ...
    public void startConnectionManagerActivity() {
        startActivityForResult(new Intent(ViewTransferActivity.this, ConnectionManagerActivity.class)
                .putExtra(ShareActivity.EXTRA_GROUP_ID, mGroup.groupId)
                .putExtra(ConnectionManagerActivity.EXTRA_ACTIVITY_SUBTITLE, getString(R.string.text_addDevicesToTransfer)), AddDevicesToTransferActivity.REQUEST_CODE_CHOOSE_DEVICE);
    }
        ...
        if (hasAnyFiles || hasRunning) {
            if (hasRunning)
                ...
                (hasIncoming == hasOutgoing) {
                    mToggleMenu.setTitle(R.string.butn_start);
                } else {
                    if (hasIncoming) {
                        mToggleMenu.setTitle(R.string.butn_receive);

                        /*ObjectAnimator anim;
                        anim = ObjectAnimator.ofInt(mToggleMenu, "backgroundColor",
                                getResources().getColor(R.color.zxing_transparent),
                                getResources().getColor(R.color.dark_colorSecondary),
                                getResources().getColor(R.color.zxing_transparent));
                        ...
                        if (getSharedPrefs()) {
                            showTipsDialog(mToggleMenu.getItemId());
                        }

                    } else {
                        mToggleMenu.setTitle(R.string.butn_send);
                    }
                }
            }
    ...
    private void showTipsDialog(int itemId) {
        /*final AlertDialog.Builder builder = new AlertDialog.Builder(ViewTransferActivity.this);

        builder.setTitle("Receive files")
                .setMessage(R.string.click_receive_butn)
                .setCancelable(false)
        ...
        new MaterialTapTargetPrompt.Builder(ViewTransferActivity.this)
                .setTarget(itemId)
                .setPrimaryText("Tap Receive")
                .setSecondaryText("Tap the Receive button to receive your pending files")
                .setBackgroundColour(getResources().getColor(R.color.dark_colorSecondary))
    ...
    private void neverShowDialog() {
        editor.putBoolean(AppUtils.showagain, false);
        editor.apply();
    }
    ...
    private void toggleTask() {
        ...
        if (assigneeList.size() > 0) {
            ...
            AppUtils.isSender = true;
            AppUtils.isReceiver = false;
            startDeviceAddingActivity();
        }
